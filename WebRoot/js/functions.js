...
function download(music) {
    ...
    download_url=mkPlayer.api+"?types=download&id="+music.id;
    console.log(download_url)

    // downloadFile(download_url, music.name + ' - ' + music.artist); //使用服务器下载后再返回，解决跨域问题
    openDownloadDialog(music.url , music.name + ' - ' + music.artist);//存在跨域问题，会直接打开链接
}
